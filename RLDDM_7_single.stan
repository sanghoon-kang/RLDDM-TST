...
data {
  ...
  vector[N] minRT;    
  vector[N] RTbound;    // lower bound or RT (e.g., 0.1 second) for stage 1
...
parameters {
  ...
  vector<lower=0, upper=5>[N] a; 
  vector<lower=0, upper=10>[N] b; //scaling parameter
  vector<lower=0, upper=1>[N] tau_raw;
}

transformed parameters{
  vector[N] tau = RTbound + (minRT-RTbound) .* tau_raw;
}
